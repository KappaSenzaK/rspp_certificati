...
function createNewAccount($mail, $tipo, $nome, $cognome, $cod_fiscale, $data, $luogo, $pw)
{
    ...
    $conn      = connect_database();
    $statement = $conn->prepare("INSERT INTO personale(mail, tipo, nome, cognome, cod_fiscale, data_nascita, luogo, stato, pw) 
...
function insertNewAttestato($mail, $descrizione = '', $tipo = 'Altro', $data_scadenza = null, $file_allegato = '')
{
    $conn = connect_database();
...
function getUsersForCuccurullo()
{
    ...
    while ($row = $results->fetch_assoc()) {
        $users[] = [
                "mail"  => $row['mail'], "nome" => $row['nome'], "cognome" => $row['cognome'], "note" => $row['note'],
                "stato" => $row['stato'], "in_servizio" => ($row['in_servizio'] == 'si') ? 'Si' : 'No',
                "c_f"   => $row['cod_fiscale'], "data" => $row['data_nascita'], "luogo" => $row['luogo'],
        ];
    }
...
function getAttestatiForCuccurullo()
{
    ...
    while ($row = $results->fetch_assoc()) {
        $users[] = [
                "mail" => $row['mail'], "tipo" => $row['tipo'], "nome" => $row['nome'], "cognome" => $row['cognome'],
                "data" => $row['data_nascita'], "luogo" => $row['luogo'], "stato" => $row['stato'],
        ];
    }
...
function getDocentiForCuccurullo()
{
    ...
    while ($row = $results->fetch_assoc()) {
        $docenti[] = [
                "mail"        => $row['mail'], "tipo" => $row['tipo'], "nome" => $row['nome'],
                "cognome"     => $row['cognome'], "note" => $row['note'], "stato" => $row['stato'],
                "in_servizio" => ($row['in_servizio'] == 'si') ? 'Si' : 'No', "c_f" => $row['c_f'],
        ];
    }
...
function getNISForCuccurullo()
{
    ...
    $sql = "
        SELECT p.mail AS mail,
               p.tipo AS tipo,
               p.nome AS nome,
               p.cognome AS cognome,
               p.note AS note,
               p.stato AS stato,
               p.in_servizio AS in_servizio,
               p.cod_fiscale AS c_f
        FROM personale p
